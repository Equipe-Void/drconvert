...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  const getIdentifiersFields = () => {
    return project.fields.filter((field) => field.isIdentifier).length;
  };
  ...
  return (
    <div className="bg-black1 h-max rounded-md min-w-[30.938rem] px-6 py-6 hover:bg-black1/85 duration-200">
      ...
      <div className="flex justify-between py-5">
        ...
        <div className="flex flex-col gap-1">
          <p className="font-bold text-white text-4xl">{project.fields.length}</p>
          <p className="font-medium text-white/35 text-xs">Campos válidos</p>
        </div>
        <div className="flex flex-col gap-1">
          <p className="font-bold text-white text-4xl">{getIdentifiersFields()}</p>
