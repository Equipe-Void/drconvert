...
import { Classification } from "@/app/_services/users/classification"; // Importe a interface Classification
...
interface ClassificationFormProps {
  ...
  classification?: Classification | null; // Adicione esta linha
}

export function ClassificationForm({ open, setOpen, classification }: ClassificationFormProps) {
      ...
      <Dialog.Portal>
        <Dialog.Overlay className="bg-black0/90 inset-0 fixed overflow-y-auto grid place-items-center p-8">
          <Dialog.Content className="bg-black1 text-black2 min-w-[28.813rem] h-auto shadow-lg shadow-black0/25 rounded-xl">
            <Dialog.Title className="flex items-center gap-4 text-white font-medium px-8 py-8 border-b border-b-gray2/30">
              ...
              <div className="flex flex-col gap-1">
                <p className="font-bold text-white text-base">
                  {(classificationStage.stage === 1 && "Classificação") || "Tipos de entrada/saída"}
                </p>
            ...
            {(classificationStage.stage === 1 && <ClassificationFormStage1 />) || (
              <ClassificationFormStage2 classification={classification} /> // Passe a classificação para o stage 2
            )}
          </Dialog.Content>
        </Dialog.Overlay>
      </Dialog.Portal>
